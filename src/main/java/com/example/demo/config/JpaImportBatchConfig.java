...
public class JpaImportBatchConfig extends BaseConfig {
	...
	public Step csvImportJpaStep() {
		return this.stepBuilderFactory.get("CsvImportJpaStep") // Builder取得
				...
				.writer(jpaWriter()).listener(this.writeListener) // writer
				.build();
	}
	...
	public Job csvImportJpaJob() {
		return this.jobBuilderFactory.get("CsvImportJpaJob") // Builderの取得
				.incrementer(new RunIdIncrementer()) // IDのインクリメント
				.start(csvImportJpaStep()) // 最初のステップ
				.build();
	}
}
